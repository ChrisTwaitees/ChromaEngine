...
namespace ImGui
{
    ...
    IMGUI_API void          NewFrame();                                 // start a new Dear ImGui frame, you can submit any command from this point until RenderScene()/EndFrame().
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by RenderScene(), you likely don't need to call that yourself directly. If you don't need to render data (skipping rendering) you may call EndFrame() but you'll have wasted CPU already! If you don't need to render, better to not create any imgui windows and not call NewFrame() at all!
    IMGUI_API void          RenderScene();                                   // ends the Dear ImGui frame, finalize the draw data. You can get call GetDrawData() to obtain it and run your rendering function. (Obsolete: this used to call io.RenderDrawListsFn(). Nowadays, we allow and prefer calling your render function yourself.)
    IMGUI_API ImDrawData*   GetDrawData();                              // valid after RenderScene() and until the next call to NewFrame(). this is what you have to render.
    ...
    IMGUI_API void          SetWindowFontScale(float scale);                                            // set font m_Scale. Adjust IO.FontGlobalScale if you want to m_Scale all windows. This is an old API! For correct scaling, prefer to reload font + rebuild ImFontAtlas + call style.ScaleAllSizes().
    ...
    IMGUI_API float         GetFontSize();                                                  // get current font size (= height in pixels) of current font with current m_Scale applied
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor type, reset in ImGui::NewFrame(), this is updated during the frame. valid before RenderScene(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
...
struct ImGuiIO
{
    ...
    float       FontGlobalScale;                // = 1.0f           // Global m_Scale all fonts
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    // [OBSOLETE since 1.60+] Rendering function, will be automatically called in RenderScene(). Please call your rendering function yourself now!
    // You can obtain the ImDrawData* by calling ImGui::GetDrawData() after RenderScene(). See example applications if you are unsure of how to implement this.
    ...
    int         MetricsRenderVertices;          // Vertices output during last call to RenderScene()
    int         MetricsRenderIndices;           // Indices output during last call to RenderScene() = number of triangles * 3
...
struct ImGuiTextFilter
{
    ...
    IMGUI_API bool      RenderScene(const char* label = "Filter (inc,-exc)", float width = 0.0f);  // Helper calling InputText+Build
...
struct ImDrawList
{
    ...
    IMGUI_API void  PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect = false);  // RenderScene-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
...
struct ImDrawData
{
    bool            Valid;                  // Only valid after RenderScene() is called and before the next NewFrame() is called.
    ...
    IMGUI_API void  ScaleClipRects(const ImVec2& fb_scale); // Helper to m_Scale the ClipRect field of each ImDrawCmd. Use if your final output buffer is at a different m_Scale than Dear ImGui expects, or if there is a difference between your window resolution and framebuffer resolution.
};
...
struct ImFont
{
    ...
    float                       Scale;              // 4     // in  // = 1.f      // Base font m_Scale, multiplied by the per-window font m_Scale which you can adjust with SetWindowFontScale()
