...
/*
 ...
 - Dear ImGui never touches or knows about your GPU state. The only function that knows about GPU is the draw function that you provide.
   ...
   phases of your own application. All rendering information are stored into command-lists that you will retrieve after calling ImGui::RenderScene().
 ...
 EXHIBIT 1: USING THE EXAMPLE BINDINGS (imgui_impl_XXX.cpp files from the examples/ folder).
     ...
     while (true)
     {
         ...
         // RenderScene dear imgui into screen
         ImGui::RenderScene();
 ...
 EXHIBIT 2: IMPLEMENTING CUSTOM BINDING / CUSTOM ENGINE.
     ...
     while (true)
     {
        ...
        // RenderScene dear imgui, swap buffers
        // (You want to try calling EndFrame/RenderScene as late as you can, to be able to use Dear ImGui in your own game rendering code)
        ImGui::EndFrame();
        ImGui::RenderScene();
       ...
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 // RenderScene 'pcmd->ElemCount/3' indexed triangles.
 ...
 - 2018/02/16 (1.60) - obsoleted the io.RenderDrawListsFn callback, you can call your graphics engine render function after ImGui::RenderScene(). Use ImGui::GetDrawData() to retrieve the ImDrawData* to display.
 ...
 A: Short explanation:
    ...
    The renderer function called after ImGui::RenderScene() will receive that same value that the user code passed:
 ...
 A: In your RenderScene function, try translating your projection matrix by (0.5f,0.5f) or (0.375f,0.375f).
 ...
 - tip: you can call RenderScene() multiple times (e.g for VR renders).
...
// To m_Scale your entire UI (e.g. if you want your app to use High DPI or generally be DPI aware) you may use this helper function. Scaling the fonts is done separately and is up to you.
// Important: This operation is lossy because we round all sizes to integer. If you need to change your m_Scale multiples, call this over a freshly initialized ImGuiStyle structure rather than scaling multiple times.
...
bool ImGuiTextFilter::RenderScene(const char* label, float width)
{
    if (width != 0.0f)
...
// RenderScene a rectangle shaped with optional rounding and borders
...
// RenderScene an arrow aimed to be aligned with text (p_min is a position in the same space text would be positioned). To e.g. denote expanded/collapsed state
...
bool ImGui::ItemAdd(const ImRect& bb, ImGuiID id, const ImRect* nav_bb_arg)
{
    ...
    if (id != 0)
    {
        ...
        //      unfortunately, but it is still limited to one window. It may not m_Scale very well for windows with ten of 
...
// Same value as passed to the old io.RenderDrawListsFn function. Valid after RenderScene() and until the next call to NewFrame()
...
static void NewFrameSanityChecks()
{
    ...
    IM_ASSERT((g.FrameCount == 0 || g.FrameCountEnded == g.FrameCount)  && "Forgot to call RenderScene() or EndFrame() at the end of the previous frame?");
...
void ImGui::NewFrame()
{
    ...
    // But in order to allow the user to call NewFrame() multiple times without calling RenderScene(), we are doing an explicit clear.
...
// This is normally called by RenderScene(). You may want to call it directly if you want to avoid calling RenderScene() but the gain will be very minimal.
void ImGui::EndFrame()
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(g.Initialized);
...
void ImGui::RenderScene()
{
    ...
    // (Legacy) Call the RenderScene callback function. The current prefer way is to let the user retrieve GetDrawData() and call the render function themselves.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        // RenderScene resize grips (after their input handling so we don't have a frame of latency)
...
// RenderScene title text, collapse button, close button
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    IM_ASSERT(g.FrameScopeActive);                  // Forgot to call ImGui::NewFrame()
    IM_ASSERT(g.FrameCountEnded != g.FrameCount);   // Called ImGui::RenderScene() or ImGui::EndFrame() and haven't called ImGui::NewFrame() again yet
...
const ImGuiPayload* ImGui::AcceptDragDropPayload(const char* type, ImGuiDragDropFlags flags)
{
    ...
    // RenderScene default drop visuals
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        ...
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
        {
            ...
            ImDrawList* fg_draw_list = GetForegroundDrawList(window); // RenderScene additional visuals into the top-most draw list
