...
namespace Chroma
{
	void AnimationLoader::AssimpLoadAnimation(std::string const& sourcePath, std::vector<Take>& takes)
	{
		// Debug
	...
	void AnimationLoader::ProcessTakes(const aiScene* scene, aiNode* rootNode, std::vector<Take>& takes)
	{
		// collect animations
	...
	std::string AnimationLoader::GetJointName(const aiNodeAnim* animNode, const aiScene* scene)
	{
