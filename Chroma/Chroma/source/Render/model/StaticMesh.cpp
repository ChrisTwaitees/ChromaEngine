...
namespace Chroma
{
	...
	void StaticMesh::SetTextureUniforms(Shader& shader)
	{
		m_Material.SetTextureUniforms(shader);
	}


	void StaticMesh::SetTransformUniforms(Shader& shader, Camera& renderCam)
	{
		m_Material.SetTransformUniforms(shader, renderCam, GetWorldTransform());
	}


	void StaticMesh::SetMaterialUniforms(Shader& shader)
	{
	...
	{
		m_Material.Use();
		SetTransformUniforms(GetShader(), renderCam);
		BindDrawVAO();
	}
